...
class MilvusVectorDatabase(AbstractVectorDatabase):
    """Milvus-backed vector database using pymilvus directly."""
...
async def create_vector_database(
    ...
    **kwargs
    ...
    Supported types: 'milvus'. Add 'qdrant', 'helixdb' as implementations are added.
    """
    if db_type.lower() == "milvus":
        ...
        return MilvusVectorDatabase(embedding_model, uri=uri, collection_name=collection_name, dimension=dimension)
    else:
        raise ValueError(f"Unsupported vector database type: {db_type}. Supported: milvus")
